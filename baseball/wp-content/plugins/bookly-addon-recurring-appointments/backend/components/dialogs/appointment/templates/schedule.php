...
<div ng-hide="loading || form.screen != 'schedule'" class="modal-body">
    <div ng-show="form.schedule.items.length">
        <div class="alert alert-danger bookly-margin-top-remove" ng-show="form.schedule.another_time.length">
            ...
            <?php _e( 'Some of the desired time slots are busy. System offers the nearest time slot instead. Click the Edit button to select another time if needed.', 'bookly-recurring-appointments' ) ?>
        </div>
            ...
            <li class="list-group-item" ng-repeat="item in form.schedule.items | startFrom: schStartingItem() | limitTo: 10">
                <div class="row">
                    ...
                    <div class="col-sm-10 text-muted" ng-show="item.deleted">
                        <?php _e( 'Deleted', 'bookly-recurring-appointments' ) ?>
                    </div>
                    <div class="col-sm-1 text-right" ng-show="item.deleted">
                        <a href class="text-muted" title="<?php esc_attr_e( 'Restore', 'bookly-recurring-appointments' ) ?>"
                    ...
                    <div class="col-sm-2" ng-hide="item.deleted || form.schedule.edit == item.index">{{schFormatTime(item.slots, item.options)}}</div>
                    <div class="col-sm-3" ng-if="form.schedule.edit == item.index">
                    ...
                    <div class="col-sm-4" ng-hide="item.deleted || form.schedule.edit == item.index">
                            <span ng-show="item.another_time">
                                <span class="dashicons dashicons-warning"></span>
                                <?php _e( 'Another time', 'bookly-recurring-appointments' ) ?>
                            </span>
                    </div>
        ...
        <div class="alert alert-danger bookly-margin-top-remove" ng-show="form.schedule.another_time.length && schNumberOfPages() > 1">
            <span class="dashicons dashicons-warning"></span>
            <?php _e( 'Another time was offered on pages', 'bookly-recurring-appointments' )?>
