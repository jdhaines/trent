...
class Ajax extends BooklyLib\Base\Ajax
{
    ...
    public static function approveSchedule()
    {
        ...
        if ( $series_entity->isLoaded() ) {
            ...
            $count = BooklyLib\Entities\CustomerAppointment::query( 'ca' )
                ...
                ->leftJoin( 'Series', 's', 's.id = a.series_id' )
                ->where( 's.token', $token )
                ...
                if ( $affected > 0 ) {
                    ...
                    foreach ( $ca_list as $ca ) {
                        if ( ! isset ( $orders[ $ca->getCustomerId() ] ) ) {
                            $series = DataHolders\Series::create( $series_entity )->addItem( DataHolders\Simple::create( $ca ) );
                            $orders[ $ca->getCustomerId() ] = DataHolders\Order::createFromItem( $series );
                        } else {
                            $orders[ $ca->getCustomerId() ]->getItem( 0 )->addItem( DataHolders\Simple::create( $ca ) );
                        }
                    }

                    foreach ( $orders as $order ) {
                        NotificationSender::sendRecurring( $order->getItem( 0 ), $order );
                    }
                }
    ...
    )
    {
        ...
        $scheduler = new Scheduler( $chain, $datetime, $until, $repeat, $params, $exclude );
