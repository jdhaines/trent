 ...
 */
abstract class NotificationSender extends BooklyLib\NotificationSender
{
    /**
    ...
    )
    {
        ...
        if ( $staff_email_notification || $staff_sms_notification || $client_email_notification || $client_sms_notification ) {
            ...
            // Prepare codes.
            $codes = BooklyLib\NotificationCodes::createForOrder( $order, $series );
            ...
            if ( $client_email_notification || $client_sms_notification ) {
                ...
                if ( $series->getCA()->getTimeZoneOffset() !== null ) {
                    $codes->appointment_start = self::_applyTimeZone( $codes->appointment_start, $series->getCA() );
                    $codes->appointment_end   = self::_applyTimeZone( $codes->appointment_end, $series->getCA() );
     ...
     *
     * @param BooklyLib\NotificationCodes $codes
     * @return bool|string
     */
    protected static function _createIcs( BooklyLib\NotificationCodes $codes )
    {
        $ics = new ICS( $codes );
