...
class Shared extends BooklyLib\Proxy\Shared
{
    ...
    {
        /** @var \Bookly\Lib\DataHolders\Booking\Series $item */
        $item = $codes->getItem();
        if ( $item->isSeries() ) {
            $schedule = array();
    ...
    public static function prepareReplaceCodes( array $codes, NotificationCodes $notification_codes, $format )
    {
        if ( is_array( $notification_codes->schedule ) ) {
            ...
            foreach ( $notification_codes->schedule as $appointment ) {
                ...
                $time       = Utils\DateTime::formatTime( $appointment['start'] );
    ...
    {
        $names['client_pending_recurring_appointment']    = __( 'Notification to customer about pending recurring appointment', 'bookly-recurring-appointments' );
        $names['staff_pending_recurring_appointment']     = __( 'Notification to staff member about pending recurring appointment', 'bookly-recurring-appointments' );
        $names['client_approved_recurring_appointment']   = __( 'Notification to customer about approved recurring appointment', 'bookly-recurring-appointments' );
        $names['staff_approved_recurring_appointment']    = __( 'Notification to staff member about approved recurring appointment', 'bookly-recurring-appointments' );
        $names['client_cancelled_recurring_appointment']  = __( 'Notification to customer about cancelled recurring appointment', 'bookly-recurring-appointments' );
        $names['staff_cancelled_recurring_appointment']   = __( 'Notification to staff member about cancelled recurring appointment ', 'bookly-recurring-appointments' );
        $names['client_rejected_recurring_appointment']   = __( 'Notification to customer about rejected recurring appointment', 'bookly-recurring-appointments' );
        $names['staff_rejected_recurring_appointment']    = __( 'Notification to staff member about rejected recurring appointment ', 'bookly-recurring-appointments' );
        $names['client_waitlisted_recurring_appointment'] = __( 'Notification to customer about placing on waiting list for recurring appointment', 'bookly-recurring-appointments' );
        $names['staff_waitlisted_recurring_appointment']  = __( 'Notification to staff member about placing on waiting list for recurring appointment ', 'bookly-recurring-appointments' );
     ...
     * @inheritdoc
     */
    public static function prepareTestNotificationCodes( BooklyLib\NotificationCodes $codes )
    {
